...
const { generateShortLivedToken, logAxiosError } = require('@lemefy/api');
const { Tools, EToolResources } = require('lemefy-data-provider');
...
const createFileSearchTool = async ({ userId, files, entity_id, fileCitations = false }) => {
  return tool(
    async ({ query }) => {
      ...
      /**
       * @param {import('lemefy-data-provider').TFile & { fromAgent?: boolean }} file
