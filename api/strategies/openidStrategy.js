...
const { hashToken, logger, tenantStorage } = require('@lemefy/data-schemas');
const { Strategy: OpenIDStrategy } = require('openid-client/passport');
const { CacheKeys, ErrorTypes, SystemRoles } = require('lemefy-data-provider');
const {
  isEnabled,
  logHeaders,
  safeStringify,
...
async function applyOpenIdRoleSync({
  ...
  const { rolePriority, fallbackRole } = user?.tenantId
    ? await tenantStorage.run({ tenantId: user.tenantId }, async () =>
        getLemefyRolesForOpenIdSync(libreChatRoles),
      )
    : await getLemefyRolesForOpenIdSync(libreChatRoles);
