...
const { ErrorTypes, FileSources } = require('lemefy-data-provider');
...
jest.mock('@lemefy/api', () => {
  ...
  return {
    ...
    getAvatarFileStrategy: jest.fn((config, fallbackStrategy) => {
      const { FileSources } = jest.requireActual('lemefy-data-provider');
...
describe('setupOpenId', () => {
  ...
  beforeEach(async () => {
    ...
    jest.clearAllMocks();
    isEnabled.mockImplementation(jest.requireActual('@lemefy/api').isEnabled);
  ...
  describe('authorizationRequestParams', () => {
    ...
    it('uses the first non-empty audience when OPENID_AUDIENCE accepts multiple JWT audiences', () => {
      process.env.OPENID_AUDIENCE = ' lemefy , control-plane-web ';
  ...
  describe('OpenID role sync', () => {
    ...
    it('re-enforces tenant login policy after role sync changes the role', async () => {
      ...
      const { isEmailDomainAllowed } = require('@lemefy/api');
