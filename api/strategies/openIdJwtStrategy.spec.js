const { SystemRoles } = require('lemefy-data-provider');
...
jest.mock('@lemefy/data-schemas', () => ({
  logger: { info: jest.fn(), warn: jest.fn(), debug: jest.fn(), error: jest.fn() },
}));
jest.mock('@lemefy/api', () => ({
  isEnabled: jest.fn(() => false),
  findOpenIDUser: jest.fn(),
  getOpenIdEmail: jest.requireActual('@lemefy/api').getOpenIdEmail,
  getOpenIdIssuer: jest.fn(() => 'https://issuer.example.com'),
  normalizeOpenIdIssuer: jest.requireActual('@lemefy/api').normalizeOpenIdIssuer,
...
describe('openIdJwtStrategy – token validation', () => {
  ...
  it('requires OpenID JWTs to match the configured client audience and issuer', () => {
    withEnv({ OPENID_CLIENT_ID: 'lemefy-client-id', OPENID_AUDIENCE: undefined }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions).toMatchObject({
      audience: 'lemefy-client-id',
  ...
  it('also accepts OPENID_AUDIENCE for providers that mint resource-bound JWTs', () => {
    withEnv({ OPENID_CLIENT_ID: 'lemefy-client-id', OPENID_AUDIENCE: 'api://lemefy' }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions).toMatchObject({
      audience: ['lemefy-client-id', 'api://lemefy'],
    });
  });

  it('uses a single OPENID_AUDIENCE value when no client ID is configured', () => {
    withEnv({ OPENID_CLIENT_ID: undefined, OPENID_AUDIENCE: 'lemefy' }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions.audience).toBe('lemefy');
  });

  it('splits comma-separated OPENID_AUDIENCE values into multiple accepted audiences', () => {
    withEnv({ OPENID_CLIENT_ID: undefined, OPENID_AUDIENCE: 'lemefy,control-plane-web' }, () => {
      openIdJwtLogin(mockOpenIdConfig);
    });

    expect(capturedStrategyOptions.audience).toEqual(['lemefy', 'control-plane-web']);
  });

  it('trims whitespace around comma-separated OPENID_AUDIENCE values', () => {
    withEnv(
      { OPENID_CLIENT_ID: undefined, OPENID_AUDIENCE: ' lemefy , control-plane-web ' },
      () => {
        openIdJwtLogin(mockOpenIdConfig);
      },
    );
  ...
  it('combines OPENID_CLIENT_ID with comma-separated OPENID_AUDIENCE values and deduplicates', () => {
    withEnv(
      { OPENID_CLIENT_ID: 'lemefy', OPENID_AUDIENCE: 'lemefy,control-plane-web' },
      () => {
        openIdJwtLogin(mockOpenIdConfig);
      },
    );
...
describe('openIdJwtStrategy – OPENID_EMAIL_CLAIM', () => {
  ...
  beforeEach(() => {
    ...
    const realFindOpenIDUser = jest.requireActual('@lemefy/api').findOpenIDUser;
