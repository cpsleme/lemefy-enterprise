...
const { CacheKeys, EModelEndpoint } = require('lemefy-data-provider');
const {
...
router.get('/:conversationId', async (req, res) => {
  ...
  try {
    const convo = await getConvo(conversationId, req.user.id, req.user.tenantId || 'default');
...
router.delete('/', configMiddleware, async (req, res) => {
  ...
  try {
    const conversationIds = Object.values(filter).flat();
    const dbResponse = await deleteConvos(conversationIds, req.user.id, req.user.tenantId || 'default');
...
router.delete('/all', configMiddleware, async (req, res) => {
  try {
    const dbResponse = await deleteConvos([], req.user.id, req.user.tenantId || 'default');
    if (Array.isArray(dbResponse?.conversationIds)) {
      await deleteAgentCheckpoints(
        dbResponse.conversationIds,
        req.config?.endpoints?.[EModelEndpoint.agents]?.checkpointer,
...
router.post('/archive', validateConvoAccess, async (req, res) => {
  ...
  try {
    const dbResponse = await upsertConvo({
      conversationId,
      userId: req?.user?.id,
      tenantId: req?.user?.tenantId || 'default',
      isArchived,
