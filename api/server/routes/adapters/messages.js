...
const { ContentTypes, isAssistantsEndpoint } = require('lemefy-data-provider');
const {
  unescapeLaTeX,
  countTokens,
  sendFeedbackScore,
...
router.get('/', async (req, res) => {
  try {
    ...
    if (conversationId && messageId) {
      const messages = await getMessages({ conversationId, messageId, userId: user, tenantId: req.user?.tenantId || 'default' });
      ...
      const searchResults = await searchMessages(search, user, tenantId, pageSize);
...
router.post('/branch', async (req, res) => {
  try {
    ...
    const [sourceMessage] = await getMessages({ messageId, userId: userId, tenantId: req.user?.tenantId || 'default' });
    if (!sourceMessage) {
    ...
    /** @type {Array<import('lemefy-data-provider').TMessageContentParts>} */
    ...
    /** @type {import('lemefy-data-provider').TMessage} */
    const newMessage = {
...
router.post('/artifact/:messageId', async (req, res) => {
  try {
    ...
    const [message] = await getMessages({ messageId, userId: req.user.id, tenantId: req.user?.tenantId || 'default' });
    if (!message) {
...
router.get('/:conversationId', prepareMessageRequestValidation, async (req, res) => {
  try {
    ...
    const messagesPromise = validation.shouldFetchMessages
      ? getMessages({ conversationId, userId: req.user.id, tenantId: req.user?.tenantId || 'default' }).then(
          (messages) => ({ messages }),
          (error) => ({ error }),
        )
      : null;
...
router.get('/:conversationId/:messageId', validateMessageReq, async (req, res) => {
  try {
    ...
    const messages = await getMessages({ conversationId, messageId, user: req.user.id });
...
router.put('/:conversationId/:messageId', validateMessageReq, async (req, res) => {
  try {
    ...
    if (index === undefined) {
      const existingMessages = await getMessages({ conversationId, messageId, user: req.user.id });
      ...
      const result = await updateMessage(messageId, req.user.id, { text, tokenCount });
    ...
    const targetMessages = await getMessages({ conversationId, messageId, user: req.user.id });
    ...
    const result = await updateMessage(messageId, req.user.id, { content: updatedContent, tokenCount });
    return res.status(200).json(result);
  } catch (error) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const updatedMessage = await updateMessage(messageId, req?.user?.id, { feedback: feedback || null });
...
router.delete('/:conversationId/:messageId', validateMessageReq, async (req, res) => {
  try {
    const { conversationId, messageId } = req.params;
    await deleteMessages([messageId], req.user.id);
    res.status(204).send();
  } catch (error) {
