...
const { getBasePath, PENDING_STALE_MS } = require('@lemefy/api');
...
jest.mock('@lemefy/api', () => {
  const actual = jest.requireActual('@lemefy/api');
  return {
    ...actual,
    MCPOAuthHandler: {
    ...
    generateCheckAccess: jest.fn(({ permissionType, permissions }) => (req, res, next) => {
      const { PermissionTypes, Permissions } = require('lemefy-data-provider');
      const isMCPUseCheck =
    ...
    // Error handling utilities (from @lemefy/api mcp/errors)
...
describe('MCP Routes', () => {
  ...
  beforeEach(() => {
    ...
    const { MCPOAuthHandler } = require('@lemefy/api');
    const { getTenantId } = require('@lemefy/data-schemas');
  ...
  describe('GET /:serverName/oauth/callback', () => {
    const { MCPOAuthHandler, MCPTokenStorage } = require('@lemefy/api');
    const { getLogStores } = require('~/cache');
    ...
    it('should handle OAuth callback successfully', async () => {
      ...
      const { Constants } = require('lemefy-data-provider');
  ...
  describe('POST /:serverName/reinitialize', () => {
    ...
    it('should handle server with custom user variables', async () => {
      ...
      require('~/config').getFlowStateManager.mockReturnValue({});
      require('~/cache').getLogStores.mockReturnValue({});
      require('@lemefy/api').getUserMCPAuthMap.mockResolvedValue({
        'mcp:test-server': {
      ...
      expect(require('@lemefy/api').getUserMCPAuthMap).toHaveBeenCalledWith({
        userId: 'test-user-id',
        servers: ['test-server'],
  ...
  describe('GET /:serverName/oauth/callback - Tenant Context', () => {
    beforeEach(() => {
      const { getTenantId, tenantStorage } = require('@lemefy/data-schemas');
      const { MCPOAuthHandler, MCPTokenStorage } = require('@lemefy/api');
