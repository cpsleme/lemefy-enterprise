...
const { EModelEndpoint, Constants, openAISettings } = require('lemefy-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Lemefy conversation');
    return importLemefyConvo;
  }
...
/**
 * Imports a Lemefy conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importLemefyConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
  userRole,
  ...
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Lemefy file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Lemefy file`, error);
    throw error;
  }
}
