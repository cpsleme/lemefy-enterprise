const { logger, redactMessage } = require('@lemefy/data-schemas');
 ...
 */
/** Native Lemefy tools that are not in the manifest */
 ...
 * @returns {Promise<{
 *   toolDefinitions?: import('@lemefy/api').LCTool[];
 *   toolRegistry?: Map<string, import('@lemefy/api').LCTool>;
 *   mcpAvailableTools?: Record<string, import('@lemefy/api').LCAvailableTools>;
 *   userMCPAuthMap?: Record<string, Record<string, string>>;
 *   hasDeferredTools?: boolean;
 * }>}
 */
async function loadToolDefinitionsWrapper({ req, res, agent, streamId = null, tool_resources }) {
  ...
  /** @type {Record<string, import('@lemefy/api').LCAvailableTools>} */
      ...
      if (!isDomainAllowed) {
        logger.warn(
          `[Actions] Domain "${action.metadata.domain}" not in allowedDomains. ` +
            `Add it to lemefy.yaml actions.allowedDomains to enable this action.`,
        );
        continue;
      }
 ...
 * @param {Record<string, import('@lemefy/api').LCAvailableTools>} [params.mcpAvailableTools] - Run-scoped MCP tool definitions
 * @param {import('@lemefy/api').RequestScopedMCPConnectionStore} [params.requestScopedConnections] - Run-scoped MCP connections
...
async function loadToolsForExecution({
  ...
  if (isPTC && toolRegistry) {
    ...
    try {
      /**
       * Lemefy threads per-request Code API auth through the agents
