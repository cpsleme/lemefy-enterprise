...
const { ErrorTypes, SystemRoles, errorsToString } = require('lemefy-data-provider');
const {
  math,
  isEnabled,
  checkEmailConfig,
...
const sendVerificationEmail = async (user) => {
  ...
  await sendEmail({
    ...
    payload: {
      appName: process.env.APP_TITLE || 'Lemefy',
 ...
 * @returns {import('@lemefy/api').CloudFrontCookieScope | null}
 */
 ...
 * @param {import('@lemefy/api').CloudFrontCookieScope & { orgId?: string }} [options={}]
 * @returns {boolean}
 */
...
const setAuthTokens = async (userId, res, _session = null, req = null) => {
  try {
    ...
    res.cookie('token_provider', 'lemefy', {
      expires: new Date(refreshTokenExpires),
      httpOnly: true,
      secure: shouldUseSecureCookie(),
      sameSite: 'strict',
