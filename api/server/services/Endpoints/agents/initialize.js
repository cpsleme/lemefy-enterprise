...
const initializeClient = async ({ req, res, signal, endpointOption }) => {
  ...
  /** @type {Map<string, import('@lemefy/api').ToolInputValidationError>} */
   ...
   *   toolRegistry?: import('@librechat/agents').LCToolRegistry,
   *   requestScopedConnections?: import('@lemefy/api').RequestScopedMCPConnectionStore,
   *   openAIApiKey?: string
   * }>}
   */
   ...
   *  @type {{ latest: import('lemefy-data-provider').TContextUsageEvent | null, count: number }} */
  const contextUsageSink = { latest: null, count: 0 };
  /** @type {Array<import('lemefy-data-provider').TTokenUsageEvent>} */
  const usageEmitSink = [];
  ...
  const {
    ...
    },
    {
      ...
      // Pass through the `@lemefy/api` exports so that tests which
      // `jest.mock('@lemefy/api')` can override the initializer/validator.
      initializeAgent,
      validateAgentModel,
    },
  );
   ...
   *  (primary fallback).
   *  @type {Map<string, import('@lemefy/api').EndpointTokenConfig | undefined>} */
