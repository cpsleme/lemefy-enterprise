...
const { genAzureEndpoint, logAxiosError, applyAxiosProxyConfig } = require('@lemefy/api');
const { extractEnvVariable, STTProviders } = require('lemefy-data-provider');
...
class STTService {
  ...
  async getProviderSchema(req) {
    ...
    if (!sttSchema) {
      throw new Error(
        'No STT schema is set. Did you configure STT in the custom config (lemefy.yaml)?',
      );
    }
