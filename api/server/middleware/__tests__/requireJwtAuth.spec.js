...
// Mock @lemefy/api — the real tenantContextMiddleware is TS and cannot be
...
jest.mock('@lemefy/api', () => {
  const { tenantStorage } = require('@lemefy/data-schemas');
  ...
  const getSafeTokenProvider = (tokenProvider) => {
    ...
    return normalized === 'openid' || normalized === 'lemefy' ? normalized : 'other';
  };
...
const { getTenantId, getUserId, logger } = require('@lemefy/data-schemas');
const {
  isEnabled,
...
describe('requireJwtAuth tenant context chaining', () => {
  ...
  it('uses OpenID JWT before Lemefy JWT when the OpenID cookie is present', async () => {
    isEnabled.mockReturnValue(true);
    mockRegisteredStrategies.add('openidJwt');
    const req = mockReq(undefined, {
...
describe('requireRumProxyAuth', () => {
  ...
  it('authenticates telemetry with the Lemefy JWT strategy without tenant or cookie refresh middleware', () => {
  ...
  it('falls back to Lemefy JWT when OpenID JWT telemetry auth fails', () => {
    isEnabled.mockReturnValue(true);
    mockRegisteredStrategies.add('openidJwt');
    const req = mockReq(undefined, {
      headers: { cookie: `token_provider=openid; openid_user_id=${signedOpenIdUserCookie()}` },
