 ...
 * @param {import('@lemefy/api').InputItem[]} input
 ...
 * @param {import('@lemefy/api').Response} response
 * @param {string} agentId
 * @returns {Promise<void>}
 */
...
const createResponse = async (req, res) => {
  ...
  try {
     ...
     * Responses API uses its own response-builder shape, so Lemefy-
    ...
    if (actuallyStreaming) {
      ...
      /** @type {Promise<import('lemefy-data-provider').TAttachment | null>[]} */
      const artifactPromises = [];
      // Use Responses API-specific callback that emits lemefy:attachment events
 ...
 * The response ID maps to a conversationId in Lemefy's storage.
 *
 * @param {import('express').Request} req
 * @param {import('express').Response} res
 */
