 ...
 * @param {Map<string, import('@lemefy/api').ToolInputValidationError>} [options.toolInputValidationErrors]
...
/**
 * Helper to write attachment events in Open Responses format (lemefy:attachment)
 * @param {ServerResponse} res - The server response object
 ...
 * Emits attachments as `lemefy:attachment` events per the Open Responses extension spec.
 *
 * @param {Object} params
 * @param {ServerRequest} params.req
 * @param {ServerResponse} params.res
    ...
    for (const file of output.artifact.files) {
      ...
      artifactPromises.push(
        (async () => {
           ...
           * and emit a follow-up `lemefy:attachment` with the same
