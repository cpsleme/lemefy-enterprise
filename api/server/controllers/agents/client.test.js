...
const { Constants, ContentTypes, EModelEndpoint } = require('lemefy-data-provider');
...
describe('AgentClient - titleConvo', () => {
  ...
  describe('buildMessages with request and agent-scoped context attachments', () => {
    ...
    beforeEach(() => {
      jest.clearAllMocks();
      mockFormatInstructions.mockResolvedValue('');
      require('@lemefy/api').countFormattedMessageTokens.mockImplementation(() => 42);
    ...
    it('persists canonical token counts while counting request file context for the prompt', async () => {
      const { countFormattedMessageTokens } = require('@lemefy/api');
      const currentFile = makeTextFile('current-file', 'current.txt', 'Current turn file body');
  ...
  describe('runMemory method', () => {
    ...
    it('should handle mixed content types correctly', async () => {
      ...
      const { ContentTypes } = require('lemefy-data-provider');
  ...
  describe('useMemory method - prelimAgent assignment', () => {
    ...
    beforeEach(() => {
      ...
      mockCheckAccess = require('@lemefy/api').checkAccess;
      mockLoadAgent = require('@lemefy/api').loadAgent;
      mockInitializeAgent = require('@lemefy/api').initializeAgent;
      mockCreateMemoryProcessor = require('@lemefy/api').createMemoryProcessor;
