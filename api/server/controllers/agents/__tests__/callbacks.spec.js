const { Tools } = require('lemefy-data-provider');
...
jest.mock('@lemefy/api', () => ({
  sendEvent: jest.fn(),
  HOST_FILE_AUTHORING_ARTIFACT_KEY: '__lemefy_file_authoring',
...
describe('createToolEndCallback', () => {
  ...
  beforeEach(() => {
    ...
    logger = require('@lemefy/data-schemas').logger;
    ...
    function makeCodeExecutionEvent({
      ...
      return {
        output: {
          ...
          artifact: {
            ...(hostFileAuthoring ? { __lemefy_file_authoring: true } : {}),
            session_id: 'sess-1',
