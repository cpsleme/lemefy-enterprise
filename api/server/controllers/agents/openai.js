...
const OpenAIChatCompletionController = async (req, res) => {
  ...
  /** @type {import('@lemefy/api').OpenAIResponseContext} — key must be `requestId` to match the type used by createChunk/buildNonStreamingResponse */
  const context = {
    created,
  ...
  try {
     ...
     * Lemefy-style card SSE events don't apply here; only the
