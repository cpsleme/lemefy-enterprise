...
const configTemplatePath = path.resolve(rootPath, 'e2e/config/lemefy.e2e.yaml');
const configPath = path.resolve(rootPath, 'e2e/.generated/lemefy.e2e.yaml');
...
const mockOverrides = {
  ...
  /** Loaded in-process by `@lemefy/api`'s `createRun` to swap in a fake model. */
...
function getBaseEnv() {
  ...
  return {
    ...
    MONGO_URI: process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/Lemefy-e2e',
  ...
  try {
    ...
    if (await waitForURL(baseURL, 1000)) {
      ...
      if (!(await waitForURL(baseURL, 120000))) {
        throw new Error(`Lemefy server did not become ready at ${baseURL}`);
      }
    }
