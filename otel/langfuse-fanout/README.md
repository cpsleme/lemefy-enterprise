...
Lemefy can send tenant-scoped agent traces to a tenant Langfuse project and
...
- the Go gateway is the only endpoint Lemefy talks to;
...
- Lemefy sends tenant traces to the local fanout gateway when
...
- The collector exports every trace to the central Langfuse project using
  ...
  the Lemefy app derives central score auth from `LANGFUSE_PUBLIC_KEY` and
  `LANGFUSE_SECRET_KEY`.
- The collector also exports tenant-enabled traces to the tenant Langfuse
  project by routing on `lemefy.langfuse.destination`, then forwarding the
  tenant `Authorization` header that Lemefy attaches to the OTLP request.
- For tenant-exportable runs, Lemefy uses a destination-scoped gateway URL
...
- Before export, the collector deletes the internal `lemefy.langfuse.*`
...
- Tenant export is conditional. Lemefy uses a destination-scoped gateway URL
  ...
  auth.
- User feedback scores use Langfuse's direct REST API from the Lemefy API
  process. Central scores use Lemefy's normal central Langfuse env config;
  tenant scores use tenant app configuration when tenant fanout is enabled.

Tenant Langfuse keys are expected to come from Lemefy app configuration, for
...
- Langfuse base URLs are startup configuration. `LANGFUSE_FANOUT_CENTRAL_BASE_URL`
  and `LANGFUSE_FANOUT_TENANT_DESTINATIONS` must be known when Lemefy and the
...
- Tenant Langfuse API keys can be added, changed, or disabled in tenant app
  configuration at runtime without restarting Lemefy or the gateway.
...
- `LANGFUSE_FANOUT_TENANT_EXPORT_DISABLED=true` can be set on Lemefy as an
...
- This supports Langfuse Cloud and self-hosted Langfuse as long as each allowed
  tenant base URL is configured at Lemefy/gateway startup. Runtime tenant
...
- Media fanout is not transactional across central and tenant projects. If one
  destination accepts `POST /api/public/media` and another fails, Lemefy sees
...
- The gateway exposes Prometheus metrics at `/metrics` using the same bearer
  token shape as Lemefy. Set `LANGFUSE_FANOUT_METRICS_SECRET`, or provide
...
```dotenv
# Used by Lemefy for central feedback scores. Set this to the same non-EU
...
Then start Lemefy with the fanout override:
...
The override builds the fanout gateway image, sets `LANGFUSE_FANOUT_ENABLED=true`, and points Lemefy at
...
`LANGFUSE_FANOUT_COLLECTOR_URL` into the Lemefy app ConfigMap when they are
not already supplied in `lemefy.configEnv`.
...
- `LANGFUSE_FANOUT_CENTRAL_BASE_URL` is also consumed by the fanout deployment only.
  For non-EU central feedback scores, set Lemefy's normal `LANGFUSE_BASE_URL`
...
- `LANGFUSE_FANOUT_COLLECTOR_URL` is the local gateway URL used by Lemefy.
