...
const DEPS_HASH_MARKER = path.join(ROOT_DIR, 'node_modules', '.lemefy-deps-hash');
...
function runTurboBuild() {
  ...
  if (flags.skipClient) {
    args.push('--filter=!@lemefy/frontend');
  }
  if (flags.force) {
    args.push('--force');
  }
