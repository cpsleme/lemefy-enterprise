...
NGINX_CONF="/tmp/lemefy-subdir-test-nginx.conf"
NGINX_PID="/tmp/lemefy-subdir-test-nginx.pid"
...
worker_processes 1;
pid /tmp/lemefy-subdir-test-nginx.pid;
error_log /tmp/lemefy-subdir-test-nginx-error.log warn;
...
http {
    access_log /tmp/lemefy-subdir-test-nginx-access.log;
...
start() {
    ...
    echo ""
    echo "=== Logs ==="
    echo "  Access: /tmp/lemefy-subdir-test-nginx-access.log"
    echo "  Error:  /tmp/lemefy-subdir-test-nginx-error.log"
    echo ""
...
stop() {
    ...
    rm -f "$NGINX_CONF" /tmp/lemefy-subdir-test-nginx-*.log
