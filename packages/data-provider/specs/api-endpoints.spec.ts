...
describe('buildLoginRedirectUrl', () => {
  ...
  it('returns plain /login for basename-prefixed /login (e.g. /lemefy/login)', () => {
    window.history.replaceState({}, '', '/lemefy/login?redirect_to=%2Fc%2Fabc');
    const result = buildLoginRedirectUrl();
    expect(result).toBe('/login');
  });

  it('returns plain /login for basename-prefixed /login sub-paths', () => {
    const result = buildLoginRedirectUrl('/lemefy/login/2fa', '', '');
    expect(result).toBe('/login');
  });
