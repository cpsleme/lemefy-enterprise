...
/** Betas Lemefy injects itself, safe to strip from persisted AMRF when a
 ...
 * three user-facing modes — `'auto'` (Lemefy decides), `'summarized'`, and
 ...
 * used when the Lemefy model ID maps to an application inference profile).
...
export const bedrockInputSchema = s.tConversationSchema
  .pick({
    /* Lemefy params; optionType: 'conversation' */
    chatProjectId: true,
    modelLabel: true,
    promptPrefix: true,
    resendFiles: true,
  ...
  .transform((data) => {
    ...
    if (
      ...
      if (!isAnthropicModel) {
         ...
         * Lemefy-generated betas (output-128k, fine-grained tool streaming);
