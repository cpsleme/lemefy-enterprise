 ...
 *
 * Mirrors a minimal subset of `isPrivateIP` from `@lemefy/api` to avoid a
...
/**
 * Mirrors the allowedAddresses parser in `@lemefy/api`'s auth helpers.
 * Kept as a local copy because the data-provider package cannot import from
 * `@lemefy/api` without creating a circular dependency. Keep the two
 ...
 * - `enabled: false` is a Lemefy-only kill switch that disables the entire
 ...
 * `(options?) => ToolApprovalHookFactory` (see `@lemefy/api`'s `registerToolApprovalHook`).
 ...
 * block, Lemefy persists checkpoints to its primary MongoDB, so resume works
...
export enum Constants {
  ...
  /** Key for the Custom Config's version (lemefy.yaml). */
