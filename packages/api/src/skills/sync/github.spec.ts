...
import { getTenantId } from '@lemefy/data-schemas';
import type {
  ISkill,
  ISkillFile,
  CreateSkillInput,
...
function makeSkillFile(
  ...
  return {
    ...
    sourceMetadata: {
      provider: 'github',
      sourceId: 'lemefy-skills',
      upstreamId: 'lemefy-skills:skills/research',
...
function makeSourceAuthorId(sourceId = 'lemefy-skills', tenantId?: string): Types.ObjectId {
...
describe('createGitHubSkillSyncRunner', () => {
  ...
  it('fails duplicate discovered skill names before publishing partial mirrors', async () => {
    ...
    expect(deps.upsertStatus).toHaveBeenLastCalledWith(
      expect.objectContaining({
        ...
        errorMessage: 'GitHub source "lemefy-skills" contains multiple skills named "duplicate"',
      }),
    );
  });
  ...
  it('uses an env-backed source token without loading a stored credential', async () => {
    ...
    try {
      ...
      expect(deps.createSkill).toHaveBeenCalledWith(
        expect.objectContaining({
          sourceMetadata: expect.objectContaining({ sourceId: 'lemefy-skills' }),
        }),
      );
    } finally {
  ...
  it('does not list or resolve server credentials and skips runs when they are disabled', async () => {
    ...
    try {
      ...
      expect(status.sources).toEqual([
        expect.objectContaining({ sourceId: 'lemefy-skills', credentialPresent: false }),
        expect.objectContaining({
  ...
  it('runs a tenant-scoped source inside its tenant context and stamps the skill tenantId', async () => {
    ...
    expect(deps.upsertStatus).toHaveBeenCalledWith(
      expect.objectContaining({ sourceId: 'lemefy-skills', tenantId: 'tenant-a' }),
    );
  ...
  it('scopes mirror cleanup to the current source and deletes only its absent upstream skills', async () => {
    ...
    const existingSkill = (upstreamId: string, _id: Types.ObjectId) => {
      const skill = makeSkill({
        ...
        source: 'github',
        sourceMetadata: { provider: 'github', sourceId: 'lemefy-skills', upstreamId },
      });
      skill._id = _id;
      return skill;
    };
    const listSkillsBySource = jest.fn(async () => [
      existingSkill('lemefy-skills:skills/research', keptId),
      existingSkill('lemefy-skills:skills/removed', staleId),
    ]);
  ...
  it('deletes stale name-conflicting mirrors after file sync and before same-commit renames', async () => {
    ...
    const staleSkill = existingSkill('lemefy-skills:skills/removed', staleId, 'renamed');
    const syncedSkill = existingSkill('lemefy-skills:skills/research', existingId, 'research');
    ...
    const deps = createDeps({
      ...
      findSkillBySourceIdentity: jest.fn(async ({ upstreamId }) =>
        upstreamId === 'lemefy-skills:skills/research' ? syncedSkill : null,
      ),
  ...
  it('does not delete stale name-conflicting mirrors before another skill file sync fails', async () => {
    ...
    const staleSkill = makeExisting('lemefy-skills:skills/removed', staleId, 'renamed');
    const syncedSkill = makeExisting('lemefy-skills:skills/research', existingId, 'research');
  ...
  it('restores a stale name-conflicting mirror when the rename update fails after deletion', async () => {
    ...
    expect(createSkill).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'renamed',
        sourceMetadata: expect.objectContaining({
          upstreamId: 'lemefy-skills:skills/removed',
        }),
      }),
    );
    expect(deps.grantPermission).toHaveBeenCalledWith(
  ...
  it("does not mirror-delete another tenant's skills from an ambient source run", async () => {
    ...
    const listSkillsBySource = jest.fn(async () => [
      makeExisting('lemefy-skills:skills/removed', ambientStaleId, undefined),
      makeExisting('lemefy-skills:skills/removed', otherTenantId, 'tenant-b'),
    ]);
    const deps = createDeps({ listSkillsBySource });
    const runner = createGitHubSkillSyncRunner(deps);
    const result = await runner.runOnce();
  ...
  it('ignores source identity matches from a different tenant bucket', async () => {
    const otherTenantSkill = makeSkill({
      ...
      author: makeSourceAuthorId('lemefy-skills', 'tenant-b'),
      authorName: 'GitHub Sync',
      frontmatter: {},
      source: 'github',
      tenantId: 'tenant-b',
  ...
  it('does not match still-discovered mirrors as moved skills when new skills sync first', async () => {
    ...
    const deps = createDeps({
      fetchFn,
      findSkillBySourceIdentity: jest.fn(async ({ upstreamId }) =>
        upstreamId === 'lemefy-skills:skills/research' ? existing : null,
      ),
    ...
    expect(deps.createSkill).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'research',
        sourceMetadata: expect.objectContaining({
          upstreamId: 'lemefy-skills:skills/new',
        }),
      }),
    );
    expect(deps.updateSkill).toHaveBeenCalledWith(
  ...
  it('reuses a same-named source mirror when a skill moves configured paths', async () => {
    const existing = makeSkill({
      ...
      sourceMetadata: {
        provider: 'github',
        sourceId: 'lemefy-skills',
        upstreamId: 'lemefy-skills:skills/old-research',
        owner: 'Lemefy',
        repo: 'skills',
        ref: 'main',
    ...
    expect(fileCalls).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          sourceMetadata: expect.objectContaining({
            upstreamId: 'lemefy-skills:skills',
          }),
        }),
        expect.objectContaining({
          relativePath: 'child.txt',
          sourceMetadata: expect.objectContaining({
            upstreamId: 'lemefy-skills:skills/child',
          }),
        }),
      ]),
    );
