...
const mockGetTenantId = jest.requireMock('@lemefy/data-schemas').getTenantId as jest.Mock;
...
describe('Code API JWT minting', () => {
  ...
  beforeEach(() => {
    ...
    process.env.CODEAPI_AUTH_PROVIDER = 'lemefy-jwt';
    ...
    process.env.CODEAPI_JWT_ISSUER = 'lemefy';
  ...
  it('mints a Code API-scoped token from canonical Lemefy JWT context', async () => {
  ...
  it('marks OpenID users authenticated by Lemefy JWT as Lemefy JWT callers', async () => {
    process.env.OPENID_REUSE_TOKENS = 'true';

    const token = await mintCodeApiToken(baseRequest({ provider: 'openid' }));
    const { claims } = decodeToken(token);

    expect(claims.principal_source).toBe('lemefy_jwt');
  });
