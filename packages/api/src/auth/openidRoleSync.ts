...
import { SystemRoles } from 'lemefy-data-provider';
...
type LemefyRolesForOpenIdSyncInput = {
  rolePriority: string[];
  fallbackRole?: string;
...
/**
 * Gets the configured Lemefy roles that OpenID role sync is allowed to assign.
 * The names are read from config, checked against storage, and returned as canonical role names.
 */
export async function getLemefyRolesForOpenIdSync(
  input: LemefyRolesForOpenIdSyncInput,
): Promise<LemefyRolesForOpenIdSync> {
...
/**
 * Chooses the Lemefy role to assign from normalized OpenID token values and validated config.
...
export function selectOpenIdRole(
  ...
  // Keep only OpenID roles that can map to configured Lemefy roles.
