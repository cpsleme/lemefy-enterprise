...
import { Tools } from 'lemefy-data-provider';
import type { MemoryArtifact } from 'lemefy-data-provider';
import { createMemoryTool, processMemory } from '../memory';

// Mock the logger
// `winston.format` must be a callable factory (real winston returns a Format
// constructor) so that `@lemefy/data-schemas` dist code can complete its
