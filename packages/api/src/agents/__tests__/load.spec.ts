...
import { Constants, FileSources } from 'lemefy-data-provider';
import { agentSchema, createMethods } from '@lemefy/data-schemas';
import type {
  Agent as LemefyAgent,
...
const deps: LoadAgentDeps = {
  getAgent: (searchParameter) => getAgent(searchParameter) as Promise<LemefyAgent | null>,
    ...
    const result = await loadAddedAgent(
      {
        ...
        primaryAgent: { id: EPHEMERAL_AGENT_ID as string, tools: ['web_search'] } as LemefyAgent,
      },
      deps,
    );
