import { encryptV3, logger } from '@lemefy/data-schemas';
import {
  EModelEndpoint,
  FileSources,
  MAX_SUBAGENT_DEPTH,
...
import type { SummarizationConfig, TEndpoint } from 'lemefy-data-provider';
import type { AppConfig } from '@lemefy/data-schemas';
import { createRun } from '~/agents/run';

// Mock winston logger — `format` must be callable so @lemefy/data-schemas
...
jest.mock('@lemefy/data-schemas', () => ({
  ...jest.requireActual('@lemefy/data-schemas'),
...
/**
 * Shape of summarization parameters used in tests. The Lemefy config
...
describe('Langfuse run config', () => {
  ...
  it('adds the explicit request tenant id to Langfuse trace metadata and tags', async () => {
    ...
    expect(callArgs.langfuse).toEqual({
      ...
      metadata: { 'lemefy.tenant.id': 'tenant-1' },
  ...
  it('falls back to a full user tenant id for direct createRun callers', async () => {
    ...
    expect(callArgs.langfuse).toEqual({
      deterministicTraceId: true,
      metadata: { 'lemefy.tenant.id': 'tenant-2' },
  ...
  it('adds tenant Langfuse credentials from tenant-scoped app config', async () => {
    ...
    expect(callArgs.langfuse).toEqual({
      ...
      metadata: { 'lemefy.tenant.id': 'tenant-1' },
      lemefyTraceAttributes: {
        'lemefy.langfuse.tenant_export.enabled': 'true',
        'lemefy.langfuse.destination': 'eu',
      },
      tags: ['tenant:tenant-1'],
    });
  });
  ...
  it('routes tenant fanout traces to the configured tenant destination', async () => {
    ...
    expect(callArgs.langfuse).toMatchObject({
      ...
      lemefyTraceAttributes: {
        'lemefy.langfuse.tenant_export.enabled': 'true',
        'lemefy.langfuse.destination': 'us',
      },
    });
  });
  ...
  it('does not append a tenant route to baseUrl when fanout is disabled', async () => {
    ...
    expect(callArgs.langfuse).not.toMatchObject({
      ...
      lemefyTraceAttributes: expect.any(Object),
    });
  });
