...
import { EModelEndpoint, EToolResources, Tools } from 'lemefy-data-provider';
import type { IMongoFile } from '@lemefy/data-schemas';
import type { Agent } from 'lemefy-data-provider';
...
// Mock logger — `format` must be a callable factory so @lemefy/data-schemas
...
function createMocks(overrides?: {
  ...
  mockExtractLemefyParams.mockReturnValue({
    resendFiles: false,
    maxContextTokens,
    modelOptions: { model },
  });
...
describe('initializeAgent — provider web_search precedence', () => {
  ...
  it('keeps Anthropic native web_search when Lemefy search is not selected', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
  ...
  it('keeps Lemefy web_search definitions when native Anthropic search is not enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.ANTHROPIC,
  ...
  it('prefers Lemefy web_search when Anthropic native search is also enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.ANTHROPIC,
      providerTools: [nativeWebSearchTool],
      loadedToolDefinitions: [libreChatWebSearchDefinition],
  ...
  it('keeps Google native search when Lemefy web_search is not selected', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
  ...
  it('prefers Lemefy web_search when Google native search is also enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.GOOGLE,
      providerTools: [nativeGoogleSearchTool],
      loadedToolDefinitions: [libreChatWebSearchDefinition],
  ...
  it('preserves the Google urlContext tool when Lemefy web_search is also enabled', async () => {
    /**
     * url_context is unrelated to web search, so the Lemefy web_search conflict
...
describe('initializeAgent — attachment scoping', () => {
  ...
  it('owner-scopes request file usage updates while preserving trusted tool files', async () => {
    ...
    mockExtractLemefyParams.mockReturnValueOnce({
      resendFiles: true,
...
describe('initializeAgent — code-generated file thread filter (regression)', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockExtractLemefyParams.mockReset();
