...
import type { IMongoFile, FileOwnerScope } from '@lemefy/data-schemas';
...
function resolveProviderToolConflicts({
  ...
  if (removed > 0) {
    logger.debug(
      `[initializeAgent] Removed ${removed} ${provider} native web search tool(s); Lemefy web_search is enabled.`,
    );
  }
...
export type InitializedAgent = Agent & {
   ...
   *  registered for this agent. Authoritative Lemefy-only signal of the
 ...
 *
 * This function is exported from @lemefy/api and replaces the CJS version from
...
export async function initializeAgent(
  ...
  const { resendFiles, maxContextTokens, modelOptions } = extractLemefyParams(
   ...
   * against tools Lemefy hasn't shipped yet (Claude Code's `edit_file`,
