...
import { ResourceType, PermissionBits, EModelEndpoint } from 'lemefy-data-provider';
import type { Agent, GraphEdge, TModelsConfig, TEndpointOption } from 'lemefy-data-provider';
...
export interface DiscoverConnectedAgentsDeps {
   ...
   * can inject their test doubles via `jest.mock('@lemefy/api')` — since
...
export async function discoverConnectedAgents(
  ...
  if (agent_ids?.length) {
     ...
     * `collectEdges` uses the `lemefy-data-provider` variant (structural
