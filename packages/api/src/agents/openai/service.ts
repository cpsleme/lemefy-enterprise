 ...
 * interacting with Lemefy agents. The agent_id is passed as the "model"
 * parameter per OpenAI spec.
 *
 * Usage:
 * ```typescript
 * import { createAgentChatCompletion } from '@lemefy/api';
 *
...
import { AgentCapabilities } from 'lemefy-data-provider';
...
/**
 * Agent type from lemefy-data-provider
 */
interface Agent {
  id: string;
  name?: string;
...
/**
 * Convert OpenAI messages to Lemefy format
 */
