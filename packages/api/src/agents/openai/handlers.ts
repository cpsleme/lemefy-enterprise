 ...
 *
 * These handlers convert Lemefy's internal graph events into OpenAI-compatible
...
/**
 * Step types from lemefy-data-provider
 */
...
export class OpenAIRunStepHandler implements EventHandler {
  ...
  handle(_event: string, data: { stepDetails?: { type?: string } }): void {
    // Run step events are primarily for Lemefy UI, we use deltas for streaming
