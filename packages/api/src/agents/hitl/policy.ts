...
import { openAIBaseSchema, googleBaseSchema, anthropicBaseSchema } from 'lemefy-data-provider';
import type { Agents, TToolApprovalPolicy } from 'lemefy-data-provider';
...
export interface ToolApprovalPolicyLayers {
  /**
   * App/endpoint policy — `endpoints.agents.toolApproval` from lemefy.yaml.
 ...
 * (`LemefyCheckpointSaver`). Otherwise the SDK installs a process-local
...
/**
 * Map a Lemefy tool-approval policy to the SDK's `ToolPolicyConfig`.
 *
 * Returns `undefined` when there's nothing to configure (so the SDK's own
 * defaults apply). The `enabled` field is Lemefy-only and stripped here —
