...
export interface HostedTool {
  type: string; // e.g., 'lemefy:web_search'
  [key: string]: unknown;
}
...
/** Attachment content types for Lemefy extensions */
export interface LemefyAttachmentContent {
  /** File ID in Lemefy storage */
  file_id?: string;
  /** Original filename */
  filename?: string;
  /** MIME type */
...
/**
 * Lemefy attachment event - custom streaming event for file/image attachments
 * Follows Open Responses extension pattern with lemefy: prefix
 */
export interface LemefyAttachmentEvent extends BaseEvent {
  type: 'lemefy:attachment';
  /** The attachment data */
  attachment: LemefyAttachmentContent;
...
/** Union of all streaming events (including Lemefy extensions) */
export type ResponseEvent =
  ...
  | ReasoningDoneEvent
  | ErrorEvent
  // Lemefy extensions (prefixed per Open Responses spec)
  | LemefyAttachmentEvent;
