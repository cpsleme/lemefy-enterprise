...
import type { TCheckpointerConfig } from 'lemefy-data-provider';
import type { RunnableConfig } from '@langchain/core/runnables';

/**
 * Durable checkpointing for human-in-the-loop (HITL) resume.
 *
 * This is the seam between Lemefy and LangGraph's checkpoint machinery. A run
 ...
 *
 * For Lemefy's agent graph (standard `Annotation`/`MessagesAnnotation` channels, no
 ...
 *
 * @param threadId - the LangGraph `thread_id` (Lemefy's conversationId).
 */
 ...
 *
 * @param threadIds - LangGraph `thread_id`s (Lemefy conversationIds); falsy entries skipped.
 */
