import { logger, webSearchKeys } from '@lemefy/data-schemas';
import { Constants, extractVariableName } from 'lemefy-data-provider';
import type { TCustomConfig } from 'lemefy-data-provider';
import type { AppConfig } from '@lemefy/data-schemas';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `lemefy.yaml` config file instead.\nMore info: https://lemefy.ai/docs/configuration/lemefy_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://www.lemefy.ai/docs/configuration/authentication/email
...
export function checkVariables(): void {
  ...
  if (hasDefaultSecrets) {
    ...
    https://www.lemefy.ai/toolkit/creds_generator
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your lemefy.yaml file, such as:
          ${key}: "\${YOUR_ENV_VAR_NAME}"
          
          Then set the actual API key in your .env file or environment variables.
          
          More info: https://www.lemefy.ai/docs/configuration/lemefy_yaml/web_search`,
        );
      }
    }
  });
