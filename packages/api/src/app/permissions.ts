import { logger, tenantStorage, SYSTEM_TENANT_ID } from '@lemefy/data-schemas';
import {
  SystemRoles,
  Permissions,
  roleDefaults,
...
import type { IRole, AppConfig } from '@lemefy/data-schemas';
...
export async function updateInterfacePermissions({
  ...
  // 1. Explicit user configuration (from lemefy.yaml)
  ...
  for (const roleName of [SystemRoles.USER, SystemRoles.ADMIN]) {
     ...
     * unless explicitly configured otherwise in lemefy.yaml.
     */
