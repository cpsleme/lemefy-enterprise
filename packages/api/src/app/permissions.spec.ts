import { loadDefaultInterface } from '@lemefy/data-schemas';
import { SystemRoles, Permissions, PermissionTypes, roleDefaults } from 'lemefy-data-provider';
import type { TConfigDefaults, TCustomConfig } from 'lemefy-data-provider';
...
describe('updateInterfacePermissions - permissions', () => {
  ...
  it('should preserve existing SHARE/SHARE_PUBLIC values when using boolean config (regression test)', async () => {
    ...
    // Config uses boolean (not object), simulating `agents: true` in lemefy.yaml
    const config = {
      interface: {
