...
describe('redisTelemetry', () => {
  ...
  it('aggregates successful and failed operations onto the request span', async () => {
    ...
    expect(attributes).toMatchObject({
      'lemefy.redis.calls': 2,
      'lemefy.redis.errors': 1,
      'lemefy.redis.operations': ['eval', 'get'],
      'lemefy.redis.use_cases': ['auth_user_doc', 'rate_limit'],
      'lemefy.redis.auth_user_doc.calls': 1,
      'lemefy.redis.auth_user_doc.errors': 0,
      'lemefy.redis.rate_limit.calls': 1,
      'lemefy.redis.rate_limit.errors': 1,
    });
    expect(attributes['lemefy.redis.duration_ms']).toEqual(expect.any(Number));
    expect(attributes['lemefy.redis.max_call_ms']).toEqual(expect.any(Number));
  });
