...
import { CacheKeys } from 'lemefy-data-provider';
...
const INSTRUMENTED_CACHE = Symbol('lemefy.redisTelemetry.instrumentedCache');
...
export function finishRedisRequestTelemetry(telemetry: RedisRequestTelemetry): void {
  ...
  telemetry.span.setAttributes({
    'lemefy.redis.calls': telemetry.calls,
    'lemefy.redis.duration_ms': roundedMilliseconds(telemetry.durationMs),
    'lemefy.redis.errors': telemetry.errors,
    'lemefy.redis.max_call_ms': roundedMilliseconds(telemetry.maxCallMs),
    'lemefy.redis.operations': [...telemetry.operations].sort(),
    'lemefy.redis.use_cases': [...telemetry.useCases.keys()].sort(),
  });
  ...
  for (const [useCase, summary] of detailedUseCases) {
    const prefix = `lemefy.redis.${useCase}`;
    telemetry.span.setAttributes({
