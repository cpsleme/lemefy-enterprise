...
type CreateMethodsType = typeof import('@lemefy/data-schemas').createMethods;
type CreateModelsType = typeof import('@lemefy/data-schemas').createModels;
type RoleBitsType = typeof import('@lemefy/data-schemas').RoleBits;
...
beforeAll(async () => {
  ...
  const dataSchemas = await import('@lemefy/data-schemas');
    ...
    it('should handle complex header with mixed safe and dangerous placeholders', async () => {
      const config: ParsedServerConfig & { headers?: Record<string, string> } = {
        ...
        headers: {
          ...
          'X-Info': 'app=lemefy;email={{LIBRECHAT_USER_EMAIL}};version=1.0',
        },
      };
      const created = await serverConfigsDB.add('temp-name', config as ParsedServerConfig, userId);
      const retrieved = await serverConfigsDB.get(created.serverName, userId);
      ...
      expect(retrievedWithHeaders?.headers?.['X-Info']).toBe('app=lemefy;email=;version=1.0');
    });
  });
