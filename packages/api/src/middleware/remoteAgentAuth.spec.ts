import type { AppConfig, IUser, UserMethods } from '@lemefy/data-schemas';
...
jest.mock('@lemefy/data-schemas', () => {
  const actual = jest.requireActual('@lemefy/data-schemas');
  return {
    ...actual,
    logger: {
      info: jest.fn(),
...
import { logger, tenantStorage } from '@lemefy/data-schemas';
  ...
  describe('when OIDC verification succeeds', () => {
    ...
    it('falls back to apiKeyMiddleware when user is not found and apiKey is enabled', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('no matching Lemefy user'),
      );
    });
