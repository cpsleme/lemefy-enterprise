...
describe('createSseStreamTelemetry', () => {
  it('records normal stream completion attributes', () => {
    ...
    expect(startSpan).toHaveBeenCalledWith(
      'lemefy.sse.stream',
      {
        ...
        attributes: expect.objectContaining({
          ...
          'lemefy.stream.id': 'stream-1',
          'lemefy.stream.resume': false,
          'lemefy.stream.route': '/api/agents/chat/stream/:streamId',
        }),
      },
      context.active(),
    );
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        'lemefy.stream.bytes.sent': Buffer.byteLength(payload),
        'lemefy.stream.chunks.count': 1,
        'lemefy.stream.completed': true,
        'lemefy.stream.end_reason': 'done',
        'lemefy.stream.error_event_emitted': false,
        'lemefy.stream.final_event_emitted': true,
        'lemefy.stream.final_event_written': true,
        'lemefy.stream.time_to_first_chunk_ms': expect.any(Number),
      }),
    );
  ...
  it('records client aborts on close before writableEnded', () => {
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        'lemefy.stream.completed': false,
        'lemefy.stream.end_reason': 'client_aborted',
      }),
    );
  ...
  it('preserves subscribe_failed as the terminal reason when an error event is sent', () => {
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        'lemefy.stream.completed': false,
        'lemefy.stream.end_reason': 'subscribe_failed',
        'lemefy.stream.error_event_emitted': true,
      }),
    );
    expect(span.setStatus).toHaveBeenCalledWith({ code: SpanStatusCode.ERROR });
  ...
  it('does not count writes after the stream span has ended', () => {
    ...
    expect(span.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        'lemefy.stream.bytes.sent': Buffer.byteLength(firstPayload),
        'lemefy.stream.chunks.count': 1,
      }),
    );
    expect(span.end).toHaveBeenCalledTimes(1);
