...
const STREAM_SPAN_NAME = 'lemefy.sse.stream';
...
class SseStreamSpanTelemetry implements SseStreamTelemetry {
  ...
  constructor({ isResume, req, res, streamId }: SseStreamTelemetryOptions) {
    this.span = trace.getTracer('lemefy.telemetry').startSpan(
      STREAM_SPAN_NAME,
      {
        ...
        attributes: {
          ...
          'lemefy.stream.id': streamId,
          'lemefy.stream.resume': isResume,
          'lemefy.stream.route': STREAM_ROUTE,
        },
      },
      context.active(),
    );
  ...
  recordHeadersFlushed(): void {
    ...
    this.span.setAttribute('lemefy.stream.headers_flushed', true);
  }
  ...
  recordWrite(payload: string, options?: { final?: boolean }): void {
    ...
    if (this.firstChunkMs === undefined) {
      ...
      this.span.setAttribute('lemefy.stream.time_to_first_chunk_ms', this.firstChunkMs);
    }
  ...
  private end(reason: StreamEndReason): void {
    ...
    const attributes: Attributes = {
      ...
      'lemefy.stream.bytes.sent': this.bytesSent,
      'lemefy.stream.chunks.count': this.chunksCount,
      'lemefy.stream.completed': reason === 'done',
      'lemefy.stream.duration_ms': performance.now() - this.startTimeMs,
      'lemefy.stream.end_reason': reason,
      'lemefy.stream.error_event_emitted': this.errorEventEmitted,
      'lemefy.stream.final_event_emitted': this.finalEventEmitted,
      'lemefy.stream.final_event_written': this.finalEventWritten,
    };

    if (this.firstChunkMs !== undefined) {
      attributes['lemefy.stream.time_to_first_chunk_ms'] = this.firstChunkMs;
    }
