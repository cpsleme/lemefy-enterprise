...
describe('getBasePath', () => {
  ...
  it('should return base path for subdirectory deployment', () => {
    ...
    expect(getBasePath()).toBe('/lemefy');
  });
  ...
  it('should handle nested subdirectories', () => {
    ...
    expect(getBasePath()).toBe('/apps/lemefy');
  });
  ...
  it('should handle URLs with query parameters', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/lemefy?param=value';
    expect(getBasePath()).toBe('/lemefy');
  });

  it('should handle URLs with fragments', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/lemefy#section';
    expect(getBasePath()).toBe('/lemefy');
  });
  ...
  it('should handle URLs with ports', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:8080/lemefy';
    expect(getBasePath()).toBe('/lemefy');
  });

  it('should handle URLs with subdomains', () => {
    process.env.DOMAIN_CLIENT = 'https://app.example.com/lemefy';
    expect(getBasePath()).toBe('/lemefy');
  });
});
