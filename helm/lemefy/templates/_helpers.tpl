...
Common labels
*/}}
{{- define "lemefy.labels" -}}
{{- include "lemefy.standardLabels" (dict "root" . "selectorLabels" (include "lemefy.selectorLabels" .)) }}
{{- end }}

{{/*
Standard labels for chart-managed workloads.
*/}}
{{- define "lemefy.standardLabels" -}}
{{- $root := .root -}}
helm.sh/chart: {{ include "lemefy.chart" $root }}
{{ .selectorLabels }}
...
Selector labels
*/}}
{{- define "lemefy.selectorLabels" -}}
app.kubernetes.io/name: {{ include "lemefy.fullname" . }}
...
*/}}
{{- define "lemefy.langfuseFanout.fullname" -}}
{{- printf "%s-langfuse-fanout" (include "lemefy.fullname" .) | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Langfuse fanout collector selector labels.
*/}}
{{- define "lemefy.langfuseFanout.selectorLabels" -}}
app.kubernetes.io/name: {{ include "lemefy.langfuseFanout.fullname" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end }}
...
*/}}
{{- define "lemefy.langfuseFanout.labels" -}}
{{- include "lemefy.standardLabels" (dict "root" . "selectorLabels" (include "lemefy.langfuseFanout.selectorLabels" .)) }}
{{- end }}

{{/*
Validate Langfuse fanout destination keys. Lemefy normalizes destination
...
*/}}
{{- define "lemefy.langfuseFanout.validateDestinationKey" -}}
{{- $name := printf "%v" . -}}
{{- if not (regexMatch "^[a-z][a-z0-9_-]*$" $name) -}}
{{- fail (printf "langfuseFanout.tenant.destinations key %q is invalid; use lowercase keys matching ^[a-z][a-z0-9_-]*$ so Lemefy trace attributes match collector routes" $name) -}}
{{- end -}}
{{- end }}
...
*/}}
{{- define "lemefy.langfuseFanout.destinationBaseUrlEnvName" -}}
...
*/}}
{{- define "lemefy.langfuseFanout.validateDestinationKeys" -}}
{{- $seenEnvNames := dict -}}
{{- range $name, $_destination := .Values.langfuseFanout.tenant.destinations -}}
{{- include "lemefy.langfuseFanout.validateDestinationKey" $name -}}
{{- $envName := include "lemefy.langfuseFanout.destinationBaseUrlEnvName" $name -}}
...
{{/*
Render the fanout destination list consumed by Lemefy and the fanout gateway.
*/}}
{{- define "lemefy.langfuseFanout.tenantDestinationsEnv" -}}
{{- include "lemefy.langfuseFanout.validateDestinationKeys" . -}}
...
*/}}
{{- define "lemefy.langfuseFanout.tenantDestinationKeysEnv" -}}
{{- include "lemefy.langfuseFanout.validateDestinationKeys" . -}}
...
app.kubernetes.io/name: {{ include "lemefy.fullname" . }}-rag
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end }}
...
*/}}
{{- define "lemefy.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "lemefy.fullname" .) .Values.serviceAccount.name }}
{{- else }}
