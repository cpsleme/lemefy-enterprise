...
#
# The fanout collector must not share the main Lemefy app selector labels.
...
RENDER_CHART_DIR="$(mktemp -d -t lemefy-fanout-chart.XXXXXX)"
RENDERED_FILE="$(mktemp -t lemefy-fanout-render.XXXXXX)"
INVALID_RENDER_ERROR="$(mktemp -t lemefy-fanout-invalid-key.XXXXXX)"
COLLISION_RENDER_ERROR="$(mktemp -t lemefy-fanout-colliding-key.XXXXXX)"
...
helm template lemefy "${RENDER_CHART_DIR}" \
...
if helm template lemefy "${RENDER_CHART_DIR}" \
  --set langfuseFanout.enabled=true \
  --set langfuseFanout.central.authHeaderSecret.name=langfuse-central \
  --set langfuseFanout.redis.uri=redis://langfuse-fanout-redis:6379 \
...
const mainService = find('Service', 'lemefy-lemefy');
const fanoutService = find('Service', 'lemefy-lemefy-langfuse-fanout');
const fanoutDeployment = find('Deployment', 'lemefy-lemefy-langfuse-fanout');
...
if (
  ...
  'http://lemefy-lemefy-langfuse-fanout.default.svc.cluster.local:4318'
) {
...
console.log('PASS: Langfuse fanout selectors are isolated from the main Lemefy Service');
NODE
