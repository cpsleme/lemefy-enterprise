# Lemefy Helm Chart
This Librechat Helm Chart provides an easy, light weight template to deploy Lemefy on Kubernetes

## Variables
In this Chart, Lemefy will only work with environment Variables. You can Specify Vars and Secret using an existing Secret (This can be generated by [creating an Env File and converting it to a Kubernetes Secret](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#-em-secret-em-) `--from-env-file`)  
...
kind: Secret
metadata:
  name: lemefy-credentials-env
  namespace: <lemefy-chart-namespace>
type: Opaque
stringData:
...
Dependant of the Model you want to use, [create Credentials in your provider](https://docs.lemefy.ai/install/configuration/ai_setup.html) and add them to the Secret:
```yaml
apiVersion: v1
kind: Secret
. . . .
...
Set `lemefy.adminPanelUrl` to the admin panel base URL used for OAuth/SSO
...
end with a trailing `/` because Lemefy appends `/auth/...` callback paths.
...
This renders `ADMIN_PANEL_URL` for Lemefy's admin OAuth flow. For OpenID SSO,
also register this Lemefy callback URL with your identity provider:

```text
https://<lemefy-domain>/api/admin/oauth/openid/callback
```
...
`LANGFUSE_FANOUT_COLLECTOR_URL` for the Lemefy app unless those values are
already provided in `lemefy.configEnv`.

Set `lemefy.configEnv.LANGFUSE_FANOUT_TENANT_EXPORT_DISABLED=true` to keep
...
map rendered into Lemefy and the fanout gateway. Tenant API keys can still be added
...
and removal of Lemefy-only routing attributes before export.
