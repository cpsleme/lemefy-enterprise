# DNS Configuration for Lemefy Helm Chart

This feature allows you to configure custom DNS settings for Lemefy pods, enabling traffic redirection to proxy servers or custom endpoints.
...
Deploy:
```bash
helm upgrade --install lemefy ./helm/lemefy -f values.yaml
```
...
```bash
helm install lemefy ./helm/lemefy \
  --set dnsPolicy="None" \
...
✅ **Integration Test**
- Works with existing Lemefy configuration
...
- Kubernetes 1.19+
- Compatible with all Lemefy deployment modes
